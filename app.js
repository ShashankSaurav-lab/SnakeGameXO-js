...
function work() {
  for (let i = snake.length - 1; i >= 0; i--) {
    if (i == 0) {
      ...
      if (foodif == true) {
        ...
        //if food not eaten, then tail will...
  ...
  if (snake[0].x == foodX && snake[0].y == foodY) {
    //generates food when snake eats food
    if (food == "gemblock") {
...
function foodController() {
  ...
  function foodCheck() {
    //check if food generated on snake
...
//leaderboard
let leaderboard =
  JSON.parse(localStorage.getItem("snakegame_leaderboard")) || [];

// Function to add a new score to leaderboard and keep top 10 scores
function addScoreToLeaderboard(scorefood) {
  leaderboard.push(scorefood);
  leaderboard.sort((a, b) => {
    return b - a;
  }); // sort by score
  if (leaderboard.length > 10) leaderboard.pop(); // keep top 10
  localStorage.setItem("snakegame_leaderboard", JSON.stringify(leaderboard)); //update thr local storage
  updateLeaderboard();
}
...
updateLeaderboard(); //update while executing

document.querySelector(".resetbtn").addEventListener("click", () => {
  //reset the score
  localStorage.setItem("snakegame_leaderboard", JSON.stringify([]));
  document.getElementById("scores").innerHTML = "";
  console.log("it worked");
});

// Updating slider in leaderboard
downbtns = document.querySelectorAll(".downbtn");
for (const btn of downbtns) {
  btn.addEventListener("click", function (e) {
    let par = this.parentElement;
    this.style.display = "none";
    par.children[2].style.display = "block";
    par.children[3].style.display = "block";
  });
}
upbtns = document.querySelectorAll(".upbtn");
for (const btn of upbtns) {
  btn.addEventListener("click", function (e) {
    let par = this.parentElement;
    this.style.display = "none";
    par.children[2].style.display = "none";
    par.children[4].style.display = "block";
  });
}
